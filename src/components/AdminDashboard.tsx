...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ guides }) => {
  ...
  // Change guide.jomax_id to guide.Jomax, based on your JSON
  const filteredGuides = guides.filter((guide) =>
    guide.Jomax?.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="bg-green-100 p-4 rounded-lg">
          ...
          <p className="text-3xl font-bold text-green-600">
            {guides.reduce((acc, guide) => acc + guide.TicketC, 0)}
          </p>
        </div>
      </div>
        ...
        <table className="min-w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentGuides.map((guide) => (
              <motion.tr
                key={guide.Jomax} // Changed to guide.Jomax
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
                <td className="px-6 py-4 whitespace-nowrap">{guide.Jomax}</td> {/* Changed to guide.Jomax */}
                <td className="px-6 py-4 whitespace-nowrap">{guide.TicketC}</td> {/* Changed to guide.TicketC */}
                <td className="px-6 py-4">{guide.TicketAssigned}</td> {/* Changed to guide.TicketAssigned */}
              </motion.tr>
            ))}
          </tbody>
        </table>
