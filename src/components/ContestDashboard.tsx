...
export const ContestDashboard: React.FC<ContestDashboardProps> = ({
  ...
  const handleGenerateWinner = () => {
    if (winners.length >= 2) return; // Prevent more than two winners
  ...
  const handleCountdownComplete = () => {
    ...
    // Use next tick to avoid updating state during render
    setTimeout(() => {
  ...
  // Handle modal open/close behavior and effect when currentWinner changes
  useEffect(() => {
    if (currentWinner) {
      // This side effect can be used to trigger modal open or other actions
    }
  }, [currentWinner]);
    ...
    <div className="text-center space-y-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        {winners.map((winner, index) => (
          <motion.div
            key={winner.guide.Jomax} // Ensure Jomax is unique
          ...
          >
            ...
            <p className="text-gray-600">Guide ID: {winner.guide.Jomax}</p>
...
//   const handleGenerateWinner = () => {
//     if (winners.length >= 2) return; // Change this limit as needed
//     setShowCountdown(true);
//   };
...
//         {winners.map((winner, index) => (
//           <motion.div
//             key={winner.guide.Jomax} // Changed to guide.Jomax
//             initial={{ scale: 0, opacity: 0 }}
//             animate={{ scale: 1, opacity: 1 }}
//             whileHover={{ scale: 1.02 }}
//             className="bg-gradient-to-br from-white to-purple-50 p-6 rounded-lg shadow-lg border border-purple-100 relative overflow-hidden group"
...
//             />
//             <h3 className="text-xl font-bold mb-2">Winner {index + 1}</h3>
//             <p className="text-gray-600">Guide ID: {winner.guide.Jomax}</p> {/* Changed to guide.Jomax */}
//             <p className="text-gray-600">Winning Ticket: {winner.ticket}</p>
//             <p className="text-purple-600 font-bold">{winner.prize}</p>
//           </motion.div>
//         ))}
...
//       <CountdownOverlay isVisible={showCountdown} onComplete={handleCountdownComplete} />
//       {currentWinner && <WinnerModal winner={currentWinner} onClose={() => setCurrentWinner(null)} />}
//     </div>
//   );
// };
